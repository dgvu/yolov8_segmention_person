...
static cv::Rect box_to_original(const cv::Rect2f& box, const LetterBoxInfo& lb, const cv::Size& orig_size) {
    ...
    x0 = std::max(0.0f, std::min((float)orig_size.width - 1, x0));
    y0 = std::max(0.0f, std::min((float)orig_size.height - 1, y0));
    x1 = std::max(0.0f, std::min((float)orig_size.width - 1, x1));
    y1 = std::max(0.0f, std::min((float)orig_size.height - 1, y1));
...
static cv::Mat mask_to_original(const cv::Mat& mask_320, const LetterBoxInfo& lb, const cv::Size& orig_size) {
    cv::Rect valid(lb.pad_x, lb.pad_y, lb.new_w, lb.new_h);

    valid.x = std::max(0, valid.x);
    valid.y = std::max(0, valid.y);
    valid.width = std::min(mask_320.cols - valid.x, valid.width);
    valid.height = std::min(mask_320.rows - valid.y, valid.height);

    cv::Mat mask_valid = mask_320(valid).clone();
...
static std::string class_name_from_id(int label) {
    if (label == 0) return "person";
...
static bool process_frame(
    ...
    for (int idx = 0; idx < (int)keep.size(); ++idx) {
        ...
        cv::Mat mask_orig = mask_to_original(mask_320, lb, img.size());
        ...
        std::string label_text = class_name_from_id(obj.label) +
